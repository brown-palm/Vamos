...
def log_qtype(data, eval, metric_logger, args):
    ...
    if "nextqa" in args.dataset:
        qtype2id= {'CH': 1, 'CW': 2, 'TN': 3, 'TC': 4, 'TP': 5, 'DL': 6, 'DC': 7, 'DO': 8}
    elif "intentqa" in args.dataset:
        qtype2id= {'CH': 1, 'CW': 2, 'TN': 3, 'TP': 4}
    else:
        return
    ...
    if 'nextqa' in args.dataset:
        ...
        metric_logger.update(n=q_freq[1][1]+ep, CH=getCount(q_freq[1]))
        metric_logger.update(n=q_freq[2][1]+ep, CW=getCount(q_freq[2]))
        metric_logger.update(n=(q_freq[3][1]+q_freq[4][1]+ep), TN_TP=(q_freq[3][0]+q_freq[4][0]) / (q_freq[3][1]+q_freq[4][1]+ep))
        metric_logger.update(n=q_freq[0][1]+ep, Total=getCount(q_freq[0]))